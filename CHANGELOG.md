...
- **Core**: Fixed metadata consistency for revert compensating events.
  - Compensating events now have stable `id` (ULID) and `created` timestamps generated by the framework.
  - Tracing fields (`correlationId`, `identifier`) are now consistently inherited from the event being reverted.
  - Correct `causationId` is now set for each compensating event in a causal tree.
  - Injected `schemaVersion` from the target flow's configuration (defaulting to 1).
  - Explicitly added `isCompensating: true` and `compensatesEventId` to event metadata for better traceability.
- **Documentation**: Updated Revert Guide and READMEs to document the automatic metadata generation behavior.
