...
interface Payload {
  ...
  cosSendData: COSSendData;
  fileSha1: string;
}

export const AttachmentCOSImageUploaded: EventFlow<Payload> = {
  domain: 'attachment',
  type: 'cosImageUploaded',
  description: 'an image was uploaded to cos',
  ...
  executor: async event => {
    ...
    const { userId, sharpMetaData, metaData, cosSendData, fileSha1 } = event.payload;
    ...
    attachment.url = 'https://' + cosSendData.Location;
    attachment.etag = cosSendData.ETag;
  ...
  receiver: (eventStoreService, eventInputArgs) =>
    eventStoreService.receiveEventInput(
      AttachmentCOSImageUploaded.domain,
      AttachmentCOSImageUploaded.type,
      eventInputArgs
    )
};
...
interface COSSendData {
  /**
   * Bucket to which the object was uploaded.
   */
  Bucket: string;
  /**
   * Region to which the object was uploaded.
   */
  Region: string;
  /**
