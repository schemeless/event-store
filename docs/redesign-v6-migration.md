...
### 8. Event IDs are generated by adapters

In v6 rc.2, adapters own event ID generation. If an event reaches `append()` or `appendToStream()` without an `id`, the adapter fills one in with a ULID before persisting it.
