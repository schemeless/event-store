...
`rebuildReadModels()` itself is still synchronous: it waits for all observer work,
including `fireAndForget` observers, before returning.
...
### 8. Event IDs are generated without mutating your input objects

If an event reaches `append()` or `appendToStream()` without an `id`, the persistence layer fills one in with a ULID before persisting it. The caller's original event object is not mutated.

That means:

- `decide()` can return pure domain events without generating IDs.
- Revert compensating events no longer need to assign their own IDs.
- Explicit IDs are still preserved for imports, migrations, and deterministic tests.
...
If you previously used UUIDs in `decide()` or revert compensation code, remove them and let the adapter generate the ID.
...
Snapshot save failures are now reported via `console.error`, or through `makeAggregateRuntime(adapter, { onSnapshotError })` if you want to plug in your own logging/metrics.

### `core.stream()` requires a stream-capable adapter

`core.stream()` now fails fast when the adapter does not implement stream queries.
It no longer falls back to a full log scan.
...
// adapter must be PgEventStoreAdapter or ExpoSqliteEventStoreAdapter (v6.0.0-rc.2+)
