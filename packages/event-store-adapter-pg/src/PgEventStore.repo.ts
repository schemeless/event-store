...
export class PgEventStoreRepo implements IEventStoreRepo {
    ...
    try {
      ...
      try {
        ...
        if (err.code === '23505' && err.constraint === this.idxStreamSequence) {
          throw new Error(
            `Migration blocked: Cannot migrate legacy NULL identifiers due to sequence collisions. Please resolve duplicates manually.`
          );
        }
        throw err;
      }
   ...
   */
  private async getStreamSequenceForUpdate(client: PoolClient, domain: string, identifier: string): Promise<number> {
          ...
          try {
            ...
            if (err.code === '23505' && err.constraint === this.idxStreamSequence) {
              try {
                ...
                // Ignore rollback errors from aborted transactions
              }
              released = true;
              client.release();
              const actualSequence = await this.getStreamSequence(domain, identifier);
