...
const applyRootEvent =
  (eventFlowMap: EventFlowMap, applyQueue: ApplyQueue, getAggregate: EventStore['getAggregate']) =>
  async ({ task, done: applyQueueDone }, drained) => {
    const createdEvent = defaultEventCreator(task.currentEvent, task.causalEvent);
    try {
      const preAppliedEvent = await makeValidateAndApply(eventFlowMap, getAggregate)(createdEvent);
...
export const applyRootEventAndCollectSucceed = (
  eventFlowMap: EventFlowMap,
  applyQueue: ApplyQueue,
  getAggregate: EventStore['getAggregate']
) =>
  applyQueue.process$.pipe(
    Rx.mergeMap(applyRootEvent(eventFlowMap, applyQueue, getAggregate)),
    Rx.scan(
